...
public class TextView extends android.widget.TextView implements CornerView {
    ...
    // TODO fix TextView onMeasure to auto resize when using shadow
    /*@Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        getPaint().getTextBounds(getText().toString(), 0, getText().length() - 1, rect);
        int desiredWidth = getSuggestedMinimumWidth() + getPaddingLeft() + getPaddingRight() + rect.width();
        int desiredHeight = getSuggestedMinimumHeight() + getPaddingTop() + getPaddingBottom() + rect.height();
